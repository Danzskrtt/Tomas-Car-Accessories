...
public class InventoryManagementController {
        ...
        colActions.setCellFactory(new Callback<TableColumn<Product, Product>, TableCell<Product, Product>>() {
            ...
            public TableCell<Product, Product> call(final TableColumn<Product, Product> param) {
                return new TableCell<Product, Product>() {
                    ...
                    private final Button btnReorder = new Button("📦 Reorder");
    ...
    private void updateStatistics() {
        ...
        if (lblStockValue != null) lblStockValue.setText("₱" + df.format(stockValue));
    ...
    private void handleReorder(Product product) {
        ...
        dialog.setContentText("Enter quantity to add to current stock (" + product.getStockQuantity() + "):");
        
        dialog.showAndWait().ifPresent(result -> {
            try {
                int addAmount = Integer.parseInt(result);
                if (addAmount <= 0) {
                    showError("Please enter a positive number to restock!");
                    return;
                }
                int newStock = product.getStockQuantity() + addAmount;
