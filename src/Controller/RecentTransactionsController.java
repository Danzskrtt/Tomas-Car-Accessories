...
public class RecentTransactionsController {
    ...
    public void initialize() {
        ...
        tableTransactions.getSelectionModel().selectedItemProperty().addListener((obs, oldValue, newValue) -> {
            if (newValue != null) {
                // Load order items for the selected transaction
    ...
    private void handleViewDetails() {
        ...
        // Display order details in an alert dialog
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Transaction Details");
        alert.setHeaderText("Order #" + selectedOrder.getOrderNumber());
        
        StringBuilder details = new StringBuilder();
        details.append("Order Number: ").append(selectedOrder.getOrderNumber()).append("\n");
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            while (rs.next()) {
                OrderItem item = new OrderItem(
                    ...
                    0.0, // discount - may not be in order_items table
                    rs.getDouble("total")
                );
                items.add(item);
            }
    ...
    private void generateReceipt(Order order, List<OrderItem> items) {
        ...
        int width = 45; // Fixed width for the receipt
        ...
        // Here you could also implement actual printer functionality
        // For now, we'll show the receipt in a dialog
