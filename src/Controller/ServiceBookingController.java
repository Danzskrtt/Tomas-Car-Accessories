...
public class ServiceBookingController {
    ...
    @FXML private Button btnServiceFeeMaintenance;
    ...
    private void initializeDatabaseTables() {
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            stmt.execute("CREATE TABLE IF NOT EXISTS service_fees (" +
    ...
    private void updateStats() {
        ...
        for (ServiceBooking booking : bookingList) {
            ...
            if (status != null) {
                if (status.equalsIgnoreCase("Scheduled") || status.equalsIgnoreCase("Pending")) {
                    scheduled++;
    ...
    private void updateButtonStates() {
        ...
        // Hide Service Settings if not admin
        if(btnServiceFeeMaintenance != null) {
            String role = Model.UserSession.getInstance().getUserRole();
            btnServiceFeeMaintenance.setVisible("Admin".equalsIgnoreCase(role));
            btnServiceFeeMaintenance.setManaged("Admin".equalsIgnoreCase(role));
        }
        ...
        if (hasSelection) {
            ...
            btnCompleteBooking.setDisable(isCompletedOrCancelled);
        }
    }
    ...
    private void handleCompleteBooking() {
        ...
        // Open the service checkout dialog instead of just marking as complete
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/View/FXML/servicecheckoutdialog.fxml"));
            Parent root = loader.load();
            
            ServiceCheckoutDialogController controller = loader.getController();
            controller.initData(selected, this);
            
            Stage stage = new Stage();
            stage.setTitle("Service POS Checkout");
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.setScene(new Scene(root));
            stage.showAndWait();
    ...
    @FXML
    private void handleServiceFeeMaintenance() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/View/FXML/servicefeemaintenance.fxml"));
            Parent root = loader.load();
            
            Stage stage = new Stage();
            stage.setTitle("Service Fee Maintenance");
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.setScene(new Scene(root));
            stage.showAndWait();
            
        } catch (Exception e) {
            showError("Error opening service fee maintenance: " + e.getMessage());
            e.printStackTrace();
        }
    }
